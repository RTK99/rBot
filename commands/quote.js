exports.run = (client, msg, args) => {
    ...
    let cch = false;
    if (args.length < 1) {
        msg.reply('You must provide a message ID');
    }

    if (!/^\d{18}$/.test(args[0])) {
        msg.reply('You must provide a valid message ID.');
    } else if (/^<#\d{18}>$|^\d{18}$/.test(args[1])) {
        ch = client.channels.get(args[1].replace('<#', '').replace('>', ''));
        cch = true;
    }
    ...
    ch.fetchMessages({ around: args[0], limit: 1 })
        .then(messages => {
            if (!messages || messages.size < 1) {
                ...
                msg.channel.send({
                    embed: client.utils.embed('', field + '\n\n' + message.toString(), [], options)
                        .setAuthor(message.author.username, message.author.avatarURL)
                        // .setTitle(field)
                });
            }
        }).catch(() => msg.reply('That message could not be found!'));
};
